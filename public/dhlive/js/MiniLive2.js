...
window.addEventListener('message', async (event) => {
  if (event.data?.type === 'selectedDhliveChanged') {
    console.log('Received selectedDhliveChanged',event.data.value);
    isPaused = true;
...
if (characterDropdown) {
  console.warn("characterDropdown 元素已找到，绑定事件监听器");
...
async function newVideoTask() {
    await videoProcessor.init("assets-teenager/01.mp4", "assets-teenager/combined_data.json.gz");
...
async function processVideoFrames() {
    ...
    // 检查视频帧是否已经解码完成
    if (post_flag) {
      window.parent.postMessage({ type: 'progress', value: videoProcessor.videoFrames.length / videoProcessor.nbSampleTotal }, '*');
    }
    if (videoProcessor.videoFrames.length / videoProcessor.nbSampleTotal > 1 ) {
      post_flag =0
    }
